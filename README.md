...
+ To build a Docker Image (this will create an image with `bhitsdev/vss:latest` tag):
...
A [SQL file](https://github.com/bhits-dev/vss/blob/master/vss-db-sample/vss_db_sample.sql) is provided with this project to populate the database with sample value set data.
...
+ Run as a Docker Container: `docker run -d bhitsdev/vss:latest <additional program arguments>`
...
+ `docker run -d bhitsdev/vss:latest --server.port=80 --spring.datasource.password=strongpassword`
...
+ `docker run -d -v "/path/on/dockerhost/ssl_keystore.keystore:/path/to/ssl_keystore.keystore" bhitsdev/vss:latest --spring.profiles.active=ssl --server.ssl.key-store=/path/to/ssl_keystore.keystore --server.ssl.key-store-password=strongkeystorepassword`
...
Please use [GitHub Issues](https://github.com/bhits-dev/vss/issues) page to report issues.
